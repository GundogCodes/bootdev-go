...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	...
	numOfProductInStock := amountInStock(productID)
	
	
	orderTotal := priceOfProduct * float64(quantity)

	if quantity > numOfProductInStock {
	...
	if quantity != 0 {
		...
		accountBalance = accountBalance - orderTotal
	}
